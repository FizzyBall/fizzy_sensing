...
import fizzy_config as fizzy_config_module
...
print("Initializing Fizzy control system with IMU 3D visualization (RANDOM mode)...")
...
## Controls: ##
# B = exit
# A = TOGGLE random wiggle mode on/off
# LT / RT = manual motor power (when random mode is OFF)
# Recording is started/stopped via the GUI record button in the gizmo window.


class XboxThread(Thread):   # creates a class that recieves xbox controller data on an eventloop bases and returns those values
    def __init__(self) -> None:
        ...
        # Random-mode toggle (A button)
        self.random_mode = False

        # Recording button callbacks (kept for GUI signal compatibility, but
        # we DO NOT bind A to start_recording anymore — A toggles random mode.)
...
def extract_euler_from_packet(packet):
    """
    Extract roll/pitch/yaw (radians) from a Fizzy data packet.
    """
...
# PyQt app + gizmo window (this gives us the GUI record button)
...
# Wait for first valid data packet
...
satpower = 0.0
rawpower = 0.0
desired_minimal_cycle_time = 0.01
...
## Main loop ##
# NOTE: Data collection happens in the gizmo's background thread. We read
# packets from gizmo_window.data_buffer rather than calling fizzy directly,
...
try:
    while True:
        ...
        # Send the most recently computed motor command
        ...
        # Pull most recent data packet from the gizmo's shared buffer
        data = None
        try:
            acquired = dashboard.data_lock.acquire(blocking=False)
            if acquired:
            ...
            print(f"Error getting data from gizmo buffer: {e}")
